...
namespace MonkeySayMonkeyGet.Mono.StarPlatinum
{
    public class StarPlatinumInstance : MonoBehaviour
    {
        ...
        public static StarPlatinumInstance CreateInstance()
        {
            var spawnedPrefab = Instantiate(Mod.assetBundle.LoadAsset<GameObject>("StarPlatinumPrefab"));
            ...
            instance.gameObject.AddComponent<SkyApplier>();

            if (PrefabDatabase.TryGetPrefab("14bbf7f0-4276-48bf-868b-317b366edd16", out var firePrefab))
            {
                var fire = Instantiate(firePrefab);
                Utils.RemovePrefabComponents(fire);

                var fireComponent = fire.GetComponentInChildren<Fire>();
